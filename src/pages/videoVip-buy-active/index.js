...
import { couponList, get_lian, vipDiscountCourse, member_reccommend, get_vipPrice,get_activevipPrice } from "../../api/videoVip"
...
class Index extends Component {
  ...
  /** 获取vip价格 */
  getVipPrice() {
    return get_activevipPrice().then(res => {
  ...
  buyVipNext = () => {
    setTimeout(() => {
      ...
      if(share_id){
        Taro.navigateTo({ url:'/pages/videoVip-buy-active-submit?share_id='+ share_id})
      }else{
        Taro.navigateTo({ url: '/pages/videoVip-buy-active-submit' })
      }
    }, 500)
  }
      ...
      <View className='videoVip videoVipBuy' >
        ...
        >
          <View className='videoVip__top'>
            ...
            <View className='btn btn--black' onClick={this.buyVipNext}>{this.state.vipPrice}元/季度 立即开通</View>
          </View>
          ...
          <View className={['btn__open ll-cell', !this.state.bottomBtn && 'hide']} onClick={this.buyVipNext}>
            ...
            <View className='ll-cell__bd'>{this.state.vipPrice}元/季度 </View>
